...
namespace rdi_kernel
{
    ...
    &nrange)
  {
    ...
    //  kernel for 1D
    determine_sibling_halfverts(n, conn, sibhvs);
    ...
    for (int b_i{0}; b_i < N; b_i++) {
      ...
      obtain_nring_curv(nid, ring, conn, sibhvs, v2hv, ngbvs, vtags, etags,
                        ngbes, &nVerts, &a__1);
    ...
    double ring, ::coder::array<int, 2U> &stcls)
  {
    ...
    for (int b_i{0}; b_i < n; b_i++) {
      int n0;
      obtain_nring_curv(b_i + 1, ring, conn, sibhvs, v2hv, ngbvs, vtags, etags,
                        ngbes, &nVerts, &a__1);
    ...
    &nrange)
  {
    ...
    for (int b_i{0}; b_i < N; b_i++) {
      ...
      obtain_nring_quad(nid, ring, conn, opphes, v2he, ngbvs, vtags, etags,
                        ngbfs, &numNbs, &a__1);
    ...
    double ring, ::coder::array<int, 2U> &stcls)
  {
    ...
    for (int b_i{0}; b_i < n; b_i++) {
      int n0;
      obtain_nring_quad(b_i + 1, ring, conn, opphes, v2he, ngbvs, vtags, etags,
                        ngbfs, &numNbs, &a__1);
    ...
    &nrange)
  {
    ...
    ::coder::array<boolean_T, 1U> etags;
    ::coder::array<boolean_T, 1U> etagsElem;
    ::coder::array<boolean_T, 1U> vtags;
    int ngbes[2048];
    int ngbvs[2048];
    int N;
    ...
    etagsElem.set_size(conn.size(0));
    loop_ub = conn.size(0);
    for (int i{0}; i < loop_ub; i++) {
      etagsElem[i] = false;
    }
    ...
    for (int b_i{0}; b_i < N; b_i++) {
      ...
      obtain_nring_vol(nid, ring, conn, sibhfs, v2hf, ngbvs, ngbes, vtags, etags,
                       etagsElem, &nVerts, &a__1);
    ...
    double ring, ::coder::array<int, 2U> &stcls)
  {
    ...
    for (int b_i{0}; b_i < n; b_i++) {
      int n0;
      obtain_nring_vol(b_i + 1, ring, conn, sibhfs, v2hf, ngbvs, ngbes, vtags,
                       etags, etagsElem, &nVerts, &a__1);
