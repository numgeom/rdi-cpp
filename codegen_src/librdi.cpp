...
namespace rdi_kernel
{
  ...
  static inline
  void update_osusop(double n, const ::coder::array<int, 2U> &stcls,
    const ::coder::array<int, 1U> &nrange, const ::coder::array<int, 1U> &n2cPtr,
    const ::coder::array<int, 1U> &n2cList, ::coder::array<int, 1U> &rowPtr, ::
    coder::array<int, 1U> &colInd, ::coder::array<double, 1U> &vals, ::coder::
    array<int, 1U> &nnzPr);
  static inline
  ...
  void wls_eno_weights(const ::coder::array<double, 2U> &us, int npoints,
    int degree, const double us_unscaled_data[], const int us_unscaled_size[2],
    const int params_pw_size[2], ::coder::array<double, 1U> &ws);
  static inline
  void wls_eno_weights(const ::coder::array<double, 2U> &us, int npoints,
    int degree, const int params_pw_size[2]);
  static inline
...
namespace rdi_kernel
{
    ...
    1U> &rdNodes, const RdiParams *params)
  {
    ...
    rdTags.set_size(mesh_xs.size(0));
    ...
    if (rdNodes.size(0) != 0) {
      ...
      b_WlsDataStruct(params->degree, params->wlsInterp0, params->wlsUseDag,
                      &b_wls.wlsObj, &b_wls.wlsWgts, b_wls.coeffs,
                      &b_wls.interp0, &b_wls.useDag);
      if (mesh_xs.size(1) == 3) {
        loop_ub = assemble_body_kernel(mesh_xs, stcls, params->degree,
          rdNodes.size(0), &b_wls);
        rdCounts.set_size(1);
        rdCounts[0] = loop_ub;
      ...
      wls.set_size(params->nThreads);
      rdCounts.set_size(params->nThreads);
      if (!params->parTask) {
        ...
        try { //try
          if (mesh_xs.size(1) == 3) {
            assemble_body_par(mesh_xs, mesh_conn, stcls, mesh_n2cPtr,
                              ...
                              mesh_parts, wls, rdTags, rdCounts, &b_mesh_xs);
          } else if (mesh_xs.size(1) == 2) {
        ...
        try { //try
          if (mesh_xs.size(1) == 3) {
            assemble_body_task(mesh_xs, mesh_conn, stcls, mesh_n2cPtr,
                               mesh_n2cList, params->degree, params->wlsInterp0,
                               params->wlsUseDag, rowPtr, colInd, vals, nnzPr,
                               mesh_parts, wls, rdTags, rdCounts);
          } else if (mesh_xs.size(1) == 2) {
            b_assemble_body_task(mesh_xs, mesh_conn, stcls, mesh_n2cPtr,
                                 ...
                                 colInd, vals, nnzPr, mesh_parts, wls, rdTags,
                                 rdCounts);
          } else {
    ...
    if (mesh_xs.size(1) == 3) {
      if (rdNodes.size(0) != 0) {
        ...
        loop_ub = assemble_surf_kernel(mesh_xs, mesh_conn, stcls, mesh_n2cPtr,
          ...
          colInd, vals, nnzPr, rdNodes, rdNodes.size(0), &a, rdTags);
        rdCounts.set_size(1);
        rdCounts[0] = loop_ub;
      } else {
        wls.set_size(params->nThreads);
...
#pragma omp parallel default(shared) num_threads(loop_ub)
          ...
          try { //try
            assemble_surf_task(mesh_xs, mesh_conn, stcls, mesh_n2cPtr,
                               ...
                               rowPtr, colInd, vals, nnzPr, mesh_parts, wls,
                               rdTags, rdCounts);
    ...
    boolean_T use_dag, int npoints)
  {
    ...
    if (weight_name_data[0] == 'U') {
      ...
      if (weight_name_data[0] == 'U') {
        ...
        //  WLS-ENO
        wls_eno_weights(wls->us, npoints, degree, weight_params_pointwise_size);
      }
    }
...
#pragma omp parallel default(shared) num_threads(nThreads)
      ...
      try { //try
        compute_surf_h_kernel(xs.size(0), conn.size(0), xs.size(1), xs, conn,
                              n2nPtr, n2nList, surfType, h, buf, nThreads);
    ...
    surfType, ::coder::array<double, 1U> &h, int nThreads)
  {
    ...
    if (nThreads <= 1) {
      compute_surf_h_kernel(xs.size(0), conn.size(0), xs.size(1), xs, conn,
                            n2nPtr, n2nList, nrms, surfType, h, buf);
    } else {
      int m2cTryBlkErrCode;
...
#pragma omp parallel default(shared) num_threads(nThreads)
      ...
      try { //try
        compute_surf_h_kernel(xs.size(0), conn.size(0), xs.size(1), xs, conn,
                              n2nPtr, n2nList, nrms, surfType, h, buf, nThreads);
...
#pragma omp single
  ...
  static void init_osusop(const ::coder::array<int, 2U> &conn, const ::coder::
    ...
    array<int, 1U> &nnzPr)
  {
    ::coder::array<double, 1U> b_buf;
    ::coder::array<int, 1U> buf;
    ...
    for (int e{0}; e < m; e++) {
      ...
      for (int b_i{0}; b_i <= npts; b_i++) {
        ...
        for (int k{0}; k < n0; k++) {
          if (!visited[stcls[k + stcls.size(1) * nid] - 1]) {
            j++;
            nodes[j] = stcls[k + stcls.size(1) * nid];
            visited[stcls[k + stcls.size(1) * nid] - 1] = true;
          }
        }
      }
      ...
      if (i - 1 > colInd.size(0)) {
        ...
        b_buf.set_size(exSpace);
        i = vals.size(0);
        vals.set_size(vals.size(0) + exSpace);
        for (loop_ub = 0; loop_ub < exSpace; loop_ub++) {
          vals[i + loop_ub] = b_buf[loop_ub];
        }
      }
...
#pragma omp single
  ...
  static void update_osusop(double n, const ::coder::array<int, 2U> &stcls,
    const ::coder::array<int, 1U> &nrange, const ::coder::array<int, 1U> &n2cPtr,
    const ::coder::array<int, 1U> &n2cList, ::coder::array<int, 1U> &rowPtr, ::
    coder::array<int, 1U> &colInd, ::coder::array<double, 1U> &vals, ::coder::
    array<int, 1U> &nnzPr)
  {
    ...
    for (int b_i{0}; b_i < N; b_i++) {
      ...
      for (int j{i}; j <= i1; j++) {
        ...
        i4 = (i3 + nnzPr[i2 - 1]) - 1;
        for (int k{i3}; k <= i4; k++) {
          visited[colInd[k - 1] - 1] = true;
        }

        n0 = stcls[(stcls.size(1) + stcls.size(1) * b_i) - 1];
        for (int k{0}; k < n0; k++) {
          if (!visited[stcls[k + stcls.size(1) * b_i] - 1]) {
            i3 = rowPtr[i2] - rowPtr[i2 - 1];
            if (nnzPr[i2 - 1] >= i3) {
              int b_n;
              int dif;
              int u1;
              loop_ub_tmp = static_cast<int>(std::round(static_cast<double>(i3) *
                1.2));
              u1 = i3 + 1;
              if (loop_ub_tmp >= u1) {
                u1 = loop_ub_tmp;
              ...
              for (loop_ub_tmp = 0; loop_ub_tmp < u1; loop_ub_tmp++) {
                b_colInd[loop_ub_tmp] = colInd[loop_ub_tmp];
              }

              for (loop_ub_tmp = 0; loop_ub_tmp < dif; loop_ub_tmp++) {
                b_colInd[loop_ub_tmp + u1] = buf1[loop_ub_tmp];
              }
              ...
              for (loop_ub_tmp = 0; loop_ub_tmp < u1; loop_ub_tmp++) {
                b_vals[loop_ub_tmp] = vals[loop_ub_tmp];
              }

              for (loop_ub_tmp = 0; loop_ub_tmp < dif; loop_ub_tmp++) {
                b_vals[loop_ub_tmp + u1] = 0.0;
              }
    ...
    int degree, const int params_pw_size[2])
  {
    ...
    wls_invdist_weights(us, npoints, 0.5 - static_cast<double>(degree < 0), b_us);
  ...
  static void wls_eno_weights(const ::coder::array<double, 2U> &us, int npoints,
    ...
    const int params_pw_size[2], ::coder::array<double, 1U> &ws)
  {
    ::coder::array<double, 2U> b_us_unscaled_data;
    ::coder::array<double, 1U> b_us;
    ...
    //  Compute hbar using ws as buffer space
    b_us_unscaled_data.set((double *)&us_unscaled_data[0], us_unscaled_size[0],
      us_unscaled_size[1]);
    compute_distances(b_us_unscaled_data, npoints, degree < 0, ws);
                       ...
                       interp0, boolean_T use_dag, int npoints)
  {
    ...
    if (use_dag) {
      ...
      if (wls->dag[i % wls_idx_0 * wls->dag.size(1) + i / wls_idx_0] != degree +
          127) {
        //  Wrapper function for building DAG in nD.
        if (us_size[1] == 2) {
    ...
    1U> &nnzPr, ::coder::array<int, 1U> &rdNodes)
  {
    ...
    if (rowPtr.size(0) == 0) {
      ...
      update_osusop(static_cast<double>(mesh->xs.size(0)), stcls, rdNodes,
                    mesh->n2cPtr, mesh->n2cList, rowPtr, colInd, vals, nnzPr);
    }
